...
const UploadCenterView = () => {
  ...
  const handleMockupDrop = async (file) => {
    ...
    try {
      ...
      updateQueueItem(id, { status: 'processing', statusText: 'Parsing with Gemini Vision...' });
      setUploadedMockups((prev) => [{ _id: res.data.mockupId, filename: file.name, status: 'pending' }, ...prev]);

      // After 3 seconds, update to show pending status
      setTimeout(() => {
        updateQueueItem(id, { status: 'done' });
      }, 3000);
    } catch (err) {
